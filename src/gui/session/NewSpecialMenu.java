 ...
 *
 */
public class NewSpecialMenu extends List implements CommandListener, Activatable {
     ...
     * @param mode
     */
    public NewSpecialMenu() {
        this( "Special", 0 );
    }
    
    public NewSpecialMenu( String title, int index ) {
    ...
    public void commandAction( Command command, Displayable displayed ) {
		if ( command == List.SELECT_COMMAND ) {
			...
			if ( session != null ) {
				...
				int keyCode = 0;
				if (index == 0) {
					new NewSpecialMenu(getString(selectedIndex), selectedIndex + 1).activate(this, done);
				}
				else if (index == 1) {

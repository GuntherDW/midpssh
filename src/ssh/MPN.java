...
public class MPN {
    ...
    /*
     * DEBUGGING: public static void dprint (BigInteger x) { if (x.words ==
     * null) System.err.print(Long.toString((long) x.ival & 0xffffffffL, 16));
     * else dprint (System.err, x.words, x.ival); } public static void dprint
     * (int[] x) { dprint (System.err, x, x.length); } public static void dprint
     * (int[] x, int len) { dprint (System.err, x, len); } public static void
     * dprint (java.io.PrintStream ps, int[] x, int len) { ps.print('('); for
     * (int i = 0; i < len; i++) { if (i > 0) ps.print (' '); ps.print ("#x" +
     * Long.toString ((long) x[i] & 0xffffffffL, 16)); } ps.print(')'); }
     */
}
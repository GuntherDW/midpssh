...
public abstract class SshIO {
	...
	private String Send_SSH_CMSG_SESSION_KEY( byte[] anti_spoofing_cookie, byte[] server_key_public_modulus,
		...
		random_bits1 = md5.digest( SshMisc
				.addArrayOfBytes( md5.digest( ( password + login ).getBytes() ), random_bits1 ) );
		random_bits2 = md5.digest( SshMisc
				.addArrayOfBytes( md5.digest( ( password + login ).getBytes() ), random_bits2 ) );

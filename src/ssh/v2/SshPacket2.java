			...
			switch ( phase_packet ) {
				...
				case PHASE_packet_length:
					...
					if ( position == 8 ) {
						/*
						 * HERE I AM
						 * Well, the crypto doesn't appear to be working. We've tested the crypto algs in Jsch so
						 * maybe we're not getting the right keys? or setting up the ciphers incorrectly?
						 * 
						 */
						...
						if ( crypto != null ) {
							packet_length_array = crypto.decrypt( this.packet_length_array );
						}
						else {
				...
				case PHASE_block:
					...
					if ( position == block.length ) { //the block is complete
						packet_length -= hmaclen; // packet_length is now payload (incl type) + padding
						byte[] decryptedBlock = new byte[block.length - hmaclen - 3]; // 3 bytes already decrypted
						byte[] data;

						// first 3 bytes already decrypted
						System.arraycopy( block, 3, decryptedBlock, 0, decryptedBlock.length );
						...
						System.arraycopy( decryptedBlock, 0, dd, 3, decryptedBlock.length );
						decryptedBlock = dd;
						...
						//data
						if ( packet_length > padlen + 1 + 1 ) {  // 1 for padding length, 1 for type

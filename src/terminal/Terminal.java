...
public class Terminal extends Canvas {
	...
	private int color[] = { 0x000000, 0xff0000, 0x00ff00, 0xffff00, // yellow
			0x0000ff, // blue
			0x00ffff, // magenta
			0x00ffff, // cyan
			0xffffff, // white
	...
	protected void paint( Graphics g ) {
		...
		synchronized ( this ) {
			...
			// KARL the y coord 1 is because with 0 it sometimes fails to draw on my SonyEricsson K700i
		}
	}
	...
	protected synchronized void redrawBackingStore() {
		...
		// The idea is that if multiple calls to redraw occur before the call to paint then we save
		...
		if ( invalid ) {
			...
			for (int l = top; l < buffer.height && l < (top + rows); l++) {
				...
				for (int c = left; c < buffer.width && c < (left + cols); c++) {
					...
					// draw the characters
					font.drawChars(g, buffer.charArray[buffer.windowBase + l], c,
							addr, (c - left) * font.width, (l - top) * font.height);
			...
			if (buffer.showcursor
				...
				g.setColor(fgcolor);
				g.fillRect((buffer.cursorX - left) * font.width, (buffer.cursorY
						- top + buffer.screenBase - buffer.windowBase)
	...
	public void setVDUBuffer(vt320 buffer) {
		// TODO KARL note this is never called, and this means that the vt320 never calls redraw on the terminal
		// it is always triggered manually, maybe we can ditch all external calls to redraw and just let the terminal
		// call them?
		this.buffer = buffer;
